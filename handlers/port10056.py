    ...
    def handle_data(self):
        ...
        # add remote
            ...
            if "address" not in payload["data"] or "name" not in payload["data"]:
                self.ls.log("Received malformed remote info from ServerNet connection %s" % self.ip)
                self.end()
                return

            exists = self.fetch_one("SELECT * FROM remotes WHERE name = ? OR address = ?", (payload["data"]["name"], payload["data"]["address"]))
            if exists:
                self.ls.log("Remote %s tried adding remote %s, but name or address already known" % (self.ip, payload["data"]["address"]))
                self.end()
            ...
            self.ls.log("Added remote %s via ServerNet connection %s" % (payload["data"]["address"], self.ip))
        ...
        # delete remote
            ...
            if not exists:
                self.ls.log("Remote %s tried removing remote %s, but not known" % (self.ip, payload["data"]["address"]))
                self.end()
            ...
            self.ls.log("Deleted remote %s via ServerNet connection %s" % (payload["data"]["address"], self.ip))
        ...
        # motd updates
            ...
            if timestamp and "updated" in payload["data"] and int(timestamp) > (payload["data"]["updated"]):
                self.ls.log("Received MOTD update from %s, but own MOTD was more recent" % self.ip)
                self.end()
